...
let calls=[];let emailFails=false;let server;
...
before(async()=>{
  server=http.createServer((request,response)=>{let body="";request.on("data",chunk=>body+=chunk);request.on("end",()=>{
    ...
    if(request.url==="/rest/v1/rpc/issue_tavern_checkout_invitation")return response.end(JSON.stringify({status:"invited",claimId:"00000000-0000-4000-8000-000000000001",name:"Robert",email:"robert@example.com",seats:3,weekendLabel:"Weekend 01 · 30 Oct to 2 Nov 2026",expiresAt:"2026-09-01T12:00:00Z"}));
    if(request.url==="/rest/v1/rpc/mark_tavern_invitation_sent")return response.end(JSON.stringify({status:"marked"}));
